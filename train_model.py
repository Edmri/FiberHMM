...
def train_HMM(emission_probs, train_arrays):
    #train HMM using datasetimport pandas as pd
from matplotlib import pyplot as plt
import numpy as np
import getopt
import sys
...
def make_emission_probs(acc_in, inacc_in, prob_adjust):
    ...
    hexamers=dict(zip(hexamers,range(len(hexamers)))) # 4096
...
def generate_training(infiles, count, read_count, random_seed, context, me_col, edge_trim, min_me, nrows):
    ...
    train_df=pd.DataFrame()
    read_count=read_count//len(infiles) # nombre de read a garder par fichier
